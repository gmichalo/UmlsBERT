...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, tui_ids=None):
        ...
        if tui_ids is None:
            tui_ids = torch.zeros(input_shape, dtype=torch.long, device=device)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def create_soft_labels(self,   labels, cui_to_words, word_to_cui):
        ...
        # todo check with mask tokens
        for i in range(0, labels.view(-1).size()[0]):
            if labels[i].item() != -100:
                ...
                target[0][[labels[i].item()]] = 1.
                input_ids_index.append(i)
                final_labels.append(target)

        return torch.cat(final_labels, dim=0), input_ids_index
